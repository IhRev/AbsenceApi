...
public class InvitationsController(ISender sender) : ControllerBase
{
    ...
    {
        var invitations = await _sender.Send(new GetUserInvitationsQuery());
    ...
    {
        var response = await _sender.Send(new InviteUserToOrganizationCommand(ivitation));
    ...
    {
        var response = await _sender.Send(new AcceptInvitationCommand(initationId, accepted));
        return response.Match<ActionResult>(
            success => Ok(),
            badRequest => NotFound(),
            accessDenied => Forbid()
